...
在此共享环境中, 1、您**不能**创建新项目，请跳过创建您的第一个项目章节；2此环境不支持Pipelines Operator,所以您需要跳过 the xref:prerequisites.adoc#openshift_pipelines[OpenShift Pipelines,role='params-link',window='_blank'] 章节.

NOTE：翻译标注 developer sandbox还有其他限制，如最多创建10个service，每隔12小时，pod会被停止。翻译标注
...
本次实验环境Developer Sandbox for Red Hat OpenShift 4.9 已经自带cli工具。在沙盒界面最顶端右侧，点击cli工具按钮，将会加载cli工具。本章下面内容跳过。
